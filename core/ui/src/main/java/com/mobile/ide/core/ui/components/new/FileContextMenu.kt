...
@Composable
fun FileContextMenu(node: FileNode, expanded: Boolean, onDismiss: () -> Unit, onAction: (FileAction) -> Unit) {
    DropdownMenu(expanded = expanded, onDismissRequest = onDismiss) {
        if (node.isDirectory) {
            DropdownMenuItem(
                ...
                onClick = {
                    onAction(FileAction.NewFile(node))
                    onDismiss()
                },
            )
            DropdownMenuItem(
                ...
                onClick = {
                    onAction(FileAction.NewFolder(node))
                    onDismiss()
                },
            )
            HorizontalDivider()
        ...
        DropdownMenuItem(
            ...
            onClick = {
                onAction(FileAction.Rename(node))
                onDismiss()
            },
        )
        DropdownMenuItem(
            ...
            onClick = {
                onAction(FileAction.CopyPath(node))
                onDismiss()
            },
        )
        HorizontalDivider()
        DropdownMenuItem(
            ...
            onClick = {
                onAction(FileAction.Delete(node))
                onDismiss()
            },
        )
    }
...
@Composable
fun NewFileDialog(parentPath: String, isFolder: Boolean, onDismiss: () -> Unit, onCreate: (name: String) -> Unit) {
...
@Composable
fun RenameDialog(node: FileNode, onDismiss: () -> Unit, onRename: (newName: String) -> Unit) {
...
@Composable
fun DeleteConfirmDialog(node: FileNode, onDismiss: () -> Unit, onConfirm: () -> Unit) {
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text("Delete ${if (node.isDirectory) "Folder" else "File"}?") },
        text = { Text("Permanently delete '${node.name}'${if (node.isDirectory) " and all its contents" else ""}?") },
        confirmButton = {
            TextButton(
                onClick = {
                    onConfirm()
