...
fun DefaultEncoding(modifier: Modifier = Modifier) {
    PreferenceLayout(label = stringResource(com.scto.mobile.ide.core.main.R.string.default_encoding), backArrowVisible = true) {
        var selectedEncoding by remember { mutableStateOf(Settings.encoding) }
        val interaction = remember { MutableInteractionSource() }

        InfoBlock(
            icon = { Icon(imageVector = Icons.Outlined.Warning, contentDescription = null) },
            text = com.scto.mobile.ide.core.main.R.string.encoding_warning.getString(),
            warning = true,
        )
            ...
            PreferenceTemplate(
                modifier =
                    modifier.clickable(indication = ripple(), interactionSource = interaction) {
                        // XedHost?.adapter?.clearAllFragments()
                ...
                title = {
                    Text(
                        ...
                        text = Charset.defaultCharset().name() + " (${stringResource(com.scto.mobile.ide.core.main.R.string.default_option)})",
                    )
                },
                enabled = true,
                applyPaddings = true,
