...
data class File(internal val javaPath: Path) {
    ...
    val name: String get() = javaPath.fileName?.toString()?.removeSuffixIfPresent(separator) ?: ""
...
inline fun <T : AutoCloseable?, R> T.use(block: (T) -> R): R {
    ...
    catch (e: Exception) { closed = true; try { this?.close() } catch (ignored: Exception) {}; throw e } 
    finally { if (!closed) this?.close() }
}