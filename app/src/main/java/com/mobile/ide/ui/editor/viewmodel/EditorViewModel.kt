...
class EditorViewModel : ViewModel() {
    ...
    fun initializePermissions(context: Context) {
        ...
        LogCatcher.permission("EditorViewModel", "Initialization", 
        ...
        if (projectPath != currentProjectPath) {
            ...
            if (indexFile.exists() && indexFile.isFile && indexFile.canRead()) {
                if (checkPermissions("Load initial file")) {
                    ...
                    LogCatcher.fileOperation("EditorViewModel", "Load initial file", indexFile.absolutePath, "Insufficient permissions")
                }
            }
        }
    }
    ...
    suspend fun saveAllModifiedFiles(context: Context) {
        withContext(Dispatchers.IO) {
            ...
            if (!checkPermissions("Save file")) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(context, context.getString(R.string.msg_storage_perm_required), Toast.LENGTH_LONG).show()
                }
                return@withContext
            }
            ...
            modifiedFiles.forEach { state ->
                try {
                    ...
                    LogCatcher.fileOperation("EditorViewModel", "Save file", state.file.absolutePath, "Failed: ${e.message}")
                }
            }
            withContext(Dispatchers.Main) {
                if (successCount > 0) {
                    Toast.makeText(context, context.getString(R.string.msg_files_saved, successCount), Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
