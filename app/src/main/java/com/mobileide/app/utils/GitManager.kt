...
class GitManager(private val termux: TermuxBridge) {
    ...
    fun init(projectPath: String): Flow<TerminalLine> = flow {
        ...
        try {
            java.io.File(projectPath, ".gitignore").writeText("""
.gradle/
build/
*.apk
*.aab
...
""".trimIndent())
            ...
            Logger.error(LogTag.GIT, "Failed to create .gitignore: ${e.message}")
        }
    ...
    fun commit(projectPath: String, message: String): Flow<TerminalLine> = flow {
        ...
        val email = runBlocking(listOf("git", "-C", projectPath, "config", "user.email")).trim()
        if (email.isEmpty()) {
            termux.executeStream(
                listOf("git", "-C", projectPath, "config", "user.email", "dev@mobileide.local"),
                projectPath
            ).collect {}
        }
        val name = runBlocking(listOf("git", "-C", projectPath, "config", "user.name")).trim()
        if (name.isEmpty()) {
            termux.executeStream(
                listOf("git", "-C", projectPath, "config", "user.name", "MobileIDE Dev"),
                projectPath
            ).collect {}
        }

        termux.executeStream(
            listOf("git", "-C", projectPath, "commit", "-m", message),
            projectPath
        ).collect { emit(it) }
    }.flowOn(Dispatchers.IO)

    // ── Log ───────────────────────────────────────────────────────────────────
    fun log(projectPath: String, limit: Int = 10): Flow<TerminalLine> = flow {
        val result = runBlocking(listOf("git", "-C", projectPath, "log", "--oneline", "--decorate", "--graph", "-$limit"))
        if (result.isBlank()) {
            emit(TerminalLine("No commits yet", LineType.OUTPUT))
        } else {
            termux.executeStream(
    ...
    private fun runBlocking(cmd: List<String>): String {
        return try {
