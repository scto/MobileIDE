...
// ─────────────────────────────────────────────────────────────────────────────
// SCOPE_MAP  — Language → TextMate scope string
// Covers all 47 grammars bundled in assets/textmate/
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// EditorTheme — the two base themes available in assets
// ─────────────────────────────────────────────────────────────────────────────

/**
 * The two editor colour themes bundled in the assets.
 *
 * AMOLED mode automatically uses `textmate/black/darcula.json` instead of
 * `textmate/darcula.json` — that selection happens inside [EditorThemeManager].
 */
enum class EditorTheme(val displayName: String, val isDark: Boolean) {
    ...
    companion object {
        fun fromIsDark(dark: Boolean): EditorTheme = if (dark) DARCULA else QUIETLIGHT
        fun fromName(name: String): EditorTheme =
            entries.firstOrNull { it.displayName.equals(name, true) } ?: DARCULA
    }
}

// ─────────────────────────────────────────────────────────────────────────────
// TextMateSetup — public facade
// ─────────────────────────────────────────────────────────────────────────────
...
object TextMateSetup {
    ...
    /** Build + assign the TextMate colour scheme for [isDark] mode. */
    ...
    /**
     * Overload kept for call sites that pass a theme display name.
     * The name is used only to determine light vs dark — the actual
     * asset file is resolved by [EditorThemeManager].
     */
    suspend fun applyTheme(
        context: Context,
        editor: CodeEditor,
    ...
    fun configureEditor(editor: CodeEditor, settings: EditorSettings) {
        runCatching {
            ...
            editor.props.deleteMultiSpaces         = settings.tabSize
            ...
            editor.getComponent(EditorAutoCompletion::class.java)
                ?.isEnabled = settings.autoComplete
...
data class EditorSettings(
    ...
    val deleteMultiSpaces: Boolean    = true,
