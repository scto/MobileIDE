...
data class ChatSession(val id: String, var title: String, val messages: List<ChatMessage>, val timestamp: Long)
...
class AICodingViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var baseUrl by
        mutableStateOf(prefs.getString("base_url", "https://api.openai.com/v1") ?: "https://api.openai.com/v1")
    ...
    // Provider Logic
    enum class ApiProvider(@StringRes val displayNameRes: Int, val defaultBaseUrl: String, val defaultModel: String) {
    ...
    fun createNewSession() {
        val newSession =
            ChatSession(
                ...
                messages = listOf(ChatMessage("assistant", INITIAL_ASSISTANT_MESSAGE_MARKER)),
    ...
    private fun updateCurrentSession() {
        ...
        if (index != -1) {
            val updatedSession =
                sessions[index].copy(messages = messages.toList(), timestamp = System.currentTimeMillis())
            ...
            if (isDefaultChatTitle(updatedSession.title)) {
                ...
                if (firstUserMsg != null) {
                    updatedSession.title =
                        firstUserMsg.content.take(30) + if (firstUserMsg.content.length > 30) "..." else ""
                }
            }
            ...
            for (i in 0 until jsonArray.length()) {
                ...
                for (j in 0 until msgsArray.length()) {
                    ...
                    val normalizedContent =
                        when {
                            role == "assistant" && isLegacyInitialAssistantMessage(content) ->
                                INITIAL_ASSISTANT_MESSAGE_MARKER
                    ...
                    msgs.add(
                        ChatMessage(
                            ...
                            reasoningContent =
                                if (msgObj.has("reasoningContent")) msgObj.getString("reasoningContent") else null,
    ...
    fun updateSettings(newApiKey: String, newBaseUrl: String, newModel: String) {
        ...
        prefs.edit { putString("api_key", apiKey).putString("base_url", baseUrl).putString("model", model) }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            for (endpoint in candidates) {
                try {
                    ...
                    val request =
                        Request.Builder().url(endpoint).addHeader("Authorization", "Bearer $apiKey").get().build()
                    ...
                    if (!response.isSuccessful) {
                        ...
                        response.close()
                        throw AiLocalizedException(
                            localizedContent(R.string.ai_error_failed_fetch_models_http, response.code, errorMsg)
                        )
                    }
                    ...
                    withContext(Dispatchers.Main) {
                        if (fetchedModels.isNotEmpty()) {
                            ...
                            messages.add(
                                localizedMessage("system", R.string.ai_status_models_fetched, fetchedModels.size)
                            )
                            ...
                            if (endpoint.contains("/v1/models") && !baseUrl.endsWith("/v1")) {
                                ...
                                messages.add(
                                    localizedMessage("system", R.string.ai_status_base_url_autocorrected, newBaseUrl)
                                )
                            }
                        } else {
            ...
            if (!success && !isFetchingModels) {
                // Already handled in catch block for the last item, but if loop finished without success (e.g. all
                // 404s)
                withContext(Dispatchers.Main) {
            ...
            try {
                ...
                response.use {
                    if (!it.isSuccessful) {
                        ...
                        throw AiLocalizedException(
                            localizedContent(R.string.ai_error_api_call_failed, it.code, errorBody)
                        )
                    }
                    ...
                    if (choices != null && choices.length() > 0) {
                        ...
                        if (endpoint.contains("/v1/chat/completions") && !baseUrl.endsWith("/v1")) {
                            val newBaseUrl = "$currentBaseUrl/v1"
                            // We can't easily update State from here since this is background thread without Main
                            // context for Compose State
