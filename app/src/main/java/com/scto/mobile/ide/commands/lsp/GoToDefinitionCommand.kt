...
import io.github.rosemoe.sora.lsp.editor.requestDefinitionAt

class GoToDefinitionCommand : LspCommand() {
    override val id: String = "lsp.go_to_definition"
    override val title: String = "go_to_definition"
    ...
    override suspend fun executeLspCommand(context: MobileIDECommandContext, tab: CodeEditorState, editor: CodeEditor, lspEditor: LspEditor) {
        try {
            val defs = lspEditor.requestDefinitionAt(editor.cursor.leftLine, editor.cursor.leftColumn)
            withContext(Dispatchers.Main) {
                val msg = if (defs.isEmpty()) "No definition found" else "Found ${defs.size} definitions (UI not implemented)"
