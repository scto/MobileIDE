...
object SetupWorker {
    suspend fun prepareEnvironment(context: Context) {
        withContext(Dispatchers.IO) {
            ...
            // 2. Copy Rootfs archive (Note: source file is rootfs.bin, saved as alpine.tar.gz)
            ...
            // Check if /etc directory exists, if not, it means extraction hasn't happened or failed
            ...
            if (!etcDir.exists()) {
                ...
                try {
                    ...
                    if (process.exitValue() != 0) {
                        // If gzip extraction fails, try without z parameter (in case some tar versions don't support it)
            ...
            // 4. Ensure init script is up-to-date (overwrite on every startup, convenient for debugging)
            binDir.mkdirs()
            libDir.mkdirs()

            // Copy Proot dependencies to local/lib (required by ReTerminal's logic)
    ...
    private fun copyAsset(context: Context, assetName: String, destFile: File) {
        // Only copy when file doesn't exist (except for scripts, which are usually small and need updating)
