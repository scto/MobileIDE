...
fun EditorSettingsScreen(onBack: () -> Unit, onNavigate: (String) -> Unit = {}) {
    // Editor settings (read-only defaults)
    val fontSize = 14f
    val tabSize = 4
    val showLineNumbers = true
    val wordWrap = false
    val autoComplete = true
    val bracketAutoClose = true
    val autoIndent = true
    val stickyScroll = false
    val highlightLine = true
    val fontPath = "fonts/JetBrainsMono-Regular.ttf"
    ...
    var bracketClose     by remember { mutableStateOf(false) }
    var autoIndent       by remember { mutableStateOf(false) }
    var stickyScroll     by remember { mutableStateOf(false) }
    var highlightLine    by remember { mutableStateOf(false) }
    var fontPath         by remember { mutableStateOf("Default") }
