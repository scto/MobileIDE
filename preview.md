...
- Modul `:language-treesitter` bietet laut status.md (2026-06-28, "Semantic Highlighting
  Engine Integration") TreeSitter-Unterstützung für Java, Kotlin, XML und Log-Dateien;
  README.md ergänzt zusätzlich C++.
- WICHTIGER VORBEFUND: Am 2026-06-29 wurde laut status.md unter Punkt 4 
  "TreeSitter Kotlin Script (KTS) Query Fixes" bereits eine Korrektur durchgeführt:
  "Replaced local Kotlin highlights schema with the official fwcd/tree-sitter-kotlin 
  highlight schema configuration. This eliminated parser errors caused by obsolete or 
  incorrect Kotlin AST nodes (like multiline_lambda_parameter and receiver_type: (_))."
  Zusätzlich wurde unter "Immediate Highlight Color Application" ein Timing-Bug 
  behoben: "Fixed a timing bug where newly opened file tabs rendered monochrome text 
  by caching the active Jetpack Compose ColorScheme and applying theme colors 
  immediately when loading TreeSitter instances."
- Ebenfalls am 2026-06-29 wurde für XML analog Punkt 3 "TreeSitter XML Grammar 
  Corrections" durchgeführt: "Configured proper sora-editor theme color mapping in 
  EditorViewModel.kt to bind the new AndroidIDE-specific XML query tags to CSS/HTML 
  theme colors." Dies dient als exaktes Referenz-Vorgehen für die jetzt nötige 
  Kotlin-Farbzuordnungs-Vervollständigung in derselben Datei `EditorViewModel.kt`.
...
- Relevante zu prüfende Dateien: die Kotlin-Highlight-Query im `:language-treesitter`-
  ...
  `EditorViewModel.kt` (enthält laut status.md 2026-06-29 das Farb-Mapping zwischen 
  TreeSitter-Capture-Tags und dem Compose-`ColorScheme`; laut `Fixing.md` importiert 
  diese Datei bereits `com.scto.mobile.ide.core.common.utils.LogCatcher`, was den 
  Modul-Pfad `app/src/main/java/com/scto/mobile/ide/...` bestätigt).
- Zusätzlicher zu prüfender Faktor: das LogCatcher-Subsystem (siehe status.md 
  2026-06-29 "LogCatcher Configuration", `com.scto.mobile.ide.core.common.utils.
  LogCatcher`) protokolliert Editor-Operationen optional ausführlich in 
  `/sdcard/MobileIDEProjects/logs/` – kann bei Aktivierung zur Diagnose der Ursache 
  des "LSP Fehler"-Indikators (Stufe 0.4) genutzt werden.
- Zur Einordnung des LSP-Kontexts: laut status.md (2026-07-04) wurde `:extension-
  languages` als vereinheitlichtes Modul für Java/Kotlin/Bash/XML-LSP-Server 
  eingeführt, mit einem "Custom Edge server wrapper" speziell für Kotlin – ein 
  möglicher Ansatzpunkt für die Ursachenanalyse der Statusanzeige "LSP Fehler" in 
  Stufe 0.4.
...
0.3. Prüfe, ob eine der nachfolgenden, in PROGRESS.md dokumentierten späteren 
   Änderungen (insbesondere Prompt 15 "Consolidate Terminal Features Module", 
   2026-07-26, das Assets nach `features/terminal/src/main/assets/` verschoben hat) 
   versehentlich Query-/Theme-Mapping-Assets des `:language-treesitter`-Moduls 
   betroffen hat, oder ob dies ein rein isoliertes, von allen Migrationen 
   unabhängiges Problem ist.
