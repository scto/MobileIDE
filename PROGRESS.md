...
*   **idesetup openssh Package Resolution**: Resolved a package manager crash inside Debian/Ubuntu environments by conditionally mapping the `openssh` package to `openssh-client openssh-server`.
...
*   **LSP Install Script Path Corrections**: Adjusted the `installScript` parameter inside the four new language servers to correctly target the `local/bin/lsp/*.sh` path structure.
