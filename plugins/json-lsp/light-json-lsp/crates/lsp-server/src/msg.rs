...
pub enum Message {
    ...
    /// A complete JSON-RPC message string, written to the wire verbatim.
    /// This bypasses serde serialization entirely.
    PreSerialized(String),
}

// Manual Serialize: PreSerialized is never serialized through serde (handled
// in _write), but we need the impl for the other variants.
impl serde::Serialize for Message {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        match self {
            Message::Request(r) => Ref::Request(r).serialize(serializer),
            Message::Response(r) => Ref::Response(r).serialize(serializer),
            Message::Notification(n) => Ref::Notification(n).serialize(serializer),
            Message::PreSerialized(_) => {
                panic!("PreSerialized messages should not go through serde")
            }
        }
    }
}
...
impl<'de> serde::Deserialize<'de> for Message {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        match Owned::deserialize(deserializer)? {
            Owned::Request(r) => Ok(Message::Request(r)),
            Owned::Response(r) => Ok(Message::Response(r)),
            Owned::Notification(n) => Ok(Message::Notification(n)),
        }
    }
}
...
impl Message {
    ...
    fn _write(&self, w: &mut dyn Write) -> io::Result<()> {
        if let Message::PreSerialized(text) = self {
            return write_msg_text(w, text);
        }
        #[derive(Serialize)]
        struct JsonRpc<'a> {
            jsonrpc: &'static str,
...
impl Response {
    ...
    /// Build a `Message::PreSerialized` for a successful response whose
    /// `result` body is already a JSON string. This avoids both the
    /// `serde_json::Value` intermediate and the serialization pass in the
    /// writer thread — the string is written to the wire verbatim.
    pub fn preserialized_ok(id: RequestId, result_json: &str) -> Message {
        let id_json = serde_json::to_string(&id).unwrap();
        Message::PreSerialized(format!(
