...
- Modul `:language-treesitter` bietet laut status.md (2026-06-28/2026-06-29) TreeSitter-
  Unterstützung für Java, Kotlin, XML, Log und C++.
- WICHTIGER VORBEFUND: Am 2026-06-29 wurde laut status.md bereits eine Korrektur 
  durchgeführt ("TreeSitter Kotlin Script (KTS) Query Fixes"): das lokale Kotlin-
  Highlights-Schema wurde durch das offizielle `fwcd/tree-sitter-kotlin`-Highlight-
  Schema ersetzt, um Parser-Fehler durch veraltete/falsche Kotlin-AST-Nodes 
  (`multiline_lambda_parameter`, `receiver_type: (_)`) zu beheben. Zusätzlich wurde 
  ein Timing-Bug behoben ("Immediate Highlight Color Application"), bei dem neu 
  geöffnete Tabs monochromen Text zeigten, indem das aktive Compose-`ColorScheme` 
  gecacht und beim Laden von TreeSitter-Instanzen sofort angewendet wird.
- TROTZ dieser vorherigen Korrektur zeigt der aktuelle Screenshot (MainActivity.kt, 
  ...
  4. Compose-Funktionsaufrufe (`enableEdgeToDetection()`, `setContent {}`, 
     `AppTheme {}`, `Scaffold(...)`) sind farblich nicht von normalen Variablen 
     unterschieden – kein `@function.call`/`@constructor`-Capture erkennbar.
...
- Dies deutet darauf hin, dass entweder (a) die 2026-06-29-Korrektur nur die 
  ...
  zwischen Import/Keyword/Type/Function-Call eingeführt wurde, oder (b) eine 
  spätere Änderung (z. B. Prompt 15 "Consolidate Terminal Features Module" am 
  2026-07-26, bei dem Assets nach `features/terminal/src/main/assets/` verschoben 
  wurden) versehentlich Query-/Theme-Mapping-Pfade für Kotlin beeinträchtigt hat, 
  oder (c) das Compose-Theme-Farb-Mapping (`EditorViewModel.kt`, siehe 2026-06-29 
  "Theme Mapping" für XML) für Kotlin nie vollständig auf die feingranularen 
  fwcd-Capture-Gruppen erweitert wurde.
- Relevante zu prüfende Dateien: die Kotlin-Highlight-Query im `:language-treesitter`-
  ...
  Capture-Tags und dem Compose-`ColorScheme`).
...
0.2. Lokalisiere in `EditorViewModel.kt` (bzw. der zuständigen Theme-Mapping-Stelle) 
   die tatsächliche Zuordnung dieser Capture-Namen zu konkreten Compose-Farben, und 
   vergleiche sie mit der in 0.1 ermittelten vollständigen Capture-Liste. Identifiziere 
   exakt, welche Capture-Gruppen im Mapping FEHLEN oder auf dieselbe Farbe wie andere, 
   semantisch unterschiedliche Capture-Gruppen zeigen (dies erklärt die im Screenshot 
   beobachtete Farbgleichheit von package/import vs. class/override/fun sowie 
   Import-Pfaden vs. echten Typnamen).

0.3. Prüfe, ob durch die Terminal-Modul-Konsolidierung (Prompt 15, 2026-07-26 – 
   Verschiebung der Assets nach `features/terminal/src/main/assets/`) versehentlich 
   auch TreeSitter-Query-Assets betroffen waren, oder ob dies ein rein isoliertes, 
   von dieser Migration unabhängiges Problem im `:language-treesitter`-Modul ist.

0.4. Kläre unabhängig davon den Grund für die Statusanzeige "LSP Fehler" (separater 
   Log-Check im Kotlin-LSP-Server-Prozess innerhalb von `:extension-languages`, z. B. 
   Start-Fehler des Kotlin-Language-Servers oder fehlender Classpath) und dokumentiere, 
   ob dies mit dem Highlighting-Problem zusammenhängt oder komplett getrennt zu 
   behandeln ist.
