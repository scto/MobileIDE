...
object GitConflictManager {
    ...
    suspend fun getConflictingFiles(projectPath: String): List<String> =
    ...
    suspend fun resolveFileConflict(projectPath: String, relativeFilePath: String, resolvedContent: String): Boolean =
        withContext(Dispatchers.IO) {
            val rootDir = File(projectPath)
            try {
    ...
    suspend fun completeMerge(
        projectPath: String,
        commitMessage: String,
        authorName: String = "MobileIDE User",
        authorEmail: String = "dev@mobileide.app",
    ): Boolean =
        withContext(Dispatchers.IO) {
            val rootDir = File(projectPath)
            try {
    ...
    suspend fun abortMerge(projectPath: String): Boolean =
        withContext(Dispatchers.IO) {
            val rootDir = File(projectPath)
            try {
                val git = Git.open(rootDir)
