...
*   **Forwaded Terminal Session initCommand**: Fixed a major bug where clicking "Installeren" or "Update" on any LSP server inside settings just spawned a normal interactive terminal without actually running the installer script. Updated `DistroManager.kt` to append `initCommand` (when present) to the terminal session arguments list, enabling the automated execution of LSP scripts inside the container on startup.
*   **Corrected idesetup JDK Path Mapping**: Fixed an issue where `idesetup` generated an invalid `JAVA_HOME` path inside `/root/etc/mobileide-environment.properties` on ARM64 devices (mapping to `-aarch64` instead of `-arm64`). Changed path detection to use shell globbing (e.g. `java-17-openjdk*`) to dynamically locate the exact directory name in `usr/lib/jvm` regardless of architecture name differences between kernel (`uname -m`) and package manager.
